...
export default function NewLeadPage() {
  const [form, setForm] = useState({ name: '', email: '', company: '', source: '', service_interest: '', budget_range: '', notes: '' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await supabase.from('leads').insert({ ...form, status: 'new', owner_id: user?.id, created_by: user?.id })
  ...
  return (
    <div className="max-w-2xl">
      <Card className="bg-card border-border">
        <CardHeader><CardTitle className="text-zo-chrome">New Lead</CardTitle></CardHeader>
        <CardContent>

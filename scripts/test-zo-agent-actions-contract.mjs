...
assert.match(intents, /create_lead: 'Generate Lead Draft'/, 'ZO_Agent button labels must include create_lead.')
...
assert.match(panel, /useRouter/, 'ZO_Agent panel must refresh or navigate after confirmed record creation.')
assert.match(panel, /router\.push\(result\.data\.href\)/, 'ZO_Agent confirmation should open the created record when the server returns a href.')
